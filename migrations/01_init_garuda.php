...
class InitGaruda extends Migration
{
    public function up(){
        DBManager::get()->exec("CREATE TABLE IF NOT EXISTS `garuda_config` (
            `institute_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `Institute`.`Institut_id`,
        ...
        DBManager::get()->exec("CREATE TABLE IF NOT EXISTS `garuda_inst_stg` (
            `institute_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `Institute`.`Institut_id`,
            `abschluss_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `abschluss`.`abschluss_id`,
            `studiengang_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `fach`.`fach_id`,
            `mkdate` INT NOT NULL DEFAULT 0,
        ...
        DBManager::get()->exec("CREATE TABLE IF NOT EXISTS `garuda_inst_inst` (
            `institute_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `Institute`.`Institut_id`,
            `rec_inst_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `Institute`.`Institut_id`,
            `mkdate` INT NOT NULL DEFAULT 0,
        ...
        DBManager::get()->exec("CREATE TABLE IF NOT EXISTS `garuda_messages` (
            ...
            `sender_id` VARCHAR(32) COLLATE latin1_bin REFERENCES `auth_user_md5`.`user_id`,
